...
function queryAndUpdateOverview(){
 ...
 li.setAttribute("id", row['id']);
                    ...
                    var liText = document.createTextNode( row['id'] + " - " + row['nome'] + " Idade: "+ row['idade']);
...
function selectUser(){
 ...
 //Remove as linhas existentes para inserção das novas
    var dataRows = document.getElementById("procurar").getElementsByClassName("data");
 
    while (dataRows.length > 0) {
        row = dataRows[0];
        document.getElementById("procurar").removeChild(row);
    };
 ...
 li.setAttribute("id", row['id']);
                    ...
                    document.getElementById("procurar").appendChild(li);
                }
            }, function(transaction, error){
                updateStatus("Erro: " + error.code + "<br>Mensagem: " + error.message);
            });
